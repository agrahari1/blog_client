...
try {
  console.log(id)
  const result = await axios.post("http://localhost:5000/deleteArtical",{id})
  setServices(result.data.data)
  console.log(result)
} catch (error) {
  console.log(error)
    ...
    <Container>
      <div className="container">
        {services?.map((curElem, Index) => {
            ...
            return(<Card className="shadow-lg mt-3" key={Index}>
            <CardBody>
              <h2>{title}</h2>
            ...
            <div className="m-2">
              ...
              <Link className="btn btn-warning ms-2" to={`/postpageupdate/${curElem._id}`}>Update</Link>
              <Button className="ms-2"  color="danger" onClick={()=>deletePost(curElem._id)}>Delete </Button>
              <Button className="ms-2"  color="primary">Publish Blog </Button>
            </div>
          </Card>);
